...
| `npm run admin -- <cmd>` | Thin HTTP client for Worker `/admin` routes. Needs `ADMIN_TOKEN` in the environment and a running Worker (`wrangler dev` or deploy). `list` / `add` / `activate` / `deactivate` are live; `test` is a stub. |
